...
volatile enum memory pending_reset = START;  // ← START = 0
...
volatile float scale_value_filtered   = 0.0; // Für extra-glatte l/h Anzeige (optional)
...
int Icon_Pos_warn[2]        = {19, 428-25}; //position ofLED icon      warning      orange
//int Icon_Pos_snowflake[2]   = {19, 480-25};  //position of Temperature out warning
...
// === EINHEITLICHES RESET-SYSTEM ===
// 0 = kein Reset
// 1 = START (Zündung ein)
// 2 = REFUEL (Tankinhalt gestiegen)
// 3 = PERIOD (Reset-Button gedrückt)
//volatile uint8_t pending_reset = 0;  // ← DIE EINZIGE Reset-Variable!
//volatile enum memory pending_reset = (enum memory)0;  // 0=none, sonst START/REFUEL/PERIOD


//volatile uint8_t reset_mode = 0;  // 0=none, 1=START, 2=REFUEL, 3=PERIOD
...
volatile float C_actual;			        	        // consumption ml, from 0x480 calculated in l/h  may be displayed also as l/100km
                     // outer temperatur in °C (multiplied by factor 10 to be able to use int)
