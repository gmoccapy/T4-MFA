...
// DEBUG  
  if(temp_page == 6){
    ...
    temp = (shutdown_timer + 600000 - millis()) / 1000 ;
...
//Serial.println("This is update values");
  ...
  switch(Data.page){
    ...
    case 0:
      ...
      if ( (motor_on == false)){ //(warnings) ||    || (batterie) 
        // do nothing
      }
      else{
        ...
        switch(Data.mode){
          ...
          case REFUEL:
            ...
            draw_dial(75, 108, temp, 1, C_dial_display_value, 24.0, F("Ab"), F("Tanken"));
            break;
...
void draw_value_speed(int Y_Pos){
  // --- EXPONENTIAL MOVING AVERAGE (EMA) für butterweiche Drehzahl ---
  static float speed_smooth = 0.0;
  speed_smooth = 0.75f * speed_smooth + 0.25f * speed;  // 75% alt + 25% neu → perfekt!

  // Runden auf 10er Schritte (wie im Original)
  int rpm_display = (int)(speed_smooth + 10) / 20 * 20;  // rundet auf nächste 10

  // Sicherheitsgrenzen
  if (rpm_display < 0) rpm_display = 0;
  if (rpm_display > 9990) rpm_display = 9990;

  // Nur neu zeichnen, wenn sich was ändert (spart CPU + kein Flackern!)
  static int last_rpm = -1;
  if (rpm_display != last_rpm) {
    last_rpm = rpm_display;
    dtostrf(rpm_display, 4, 0, TFT_String);
    draw_value_box(Y_Pos, 134, 50, TFT_String);
  }
}
...
void draw_value_range(int Y_Pos){
  ...
  //Verbrauch pro km
  float verbrauch_ml_pro_km = C_last_25_km / 25.0;
